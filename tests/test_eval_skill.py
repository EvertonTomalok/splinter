...
def test_cli_run_kwargs_includes_eval_flags() -> None:
    ...
    with patch("splinter.pipeline.run_pipeline", return_value=0) as mock_run:
        app(args=["run", "--task", "samples/hello-world-task.yaml",
                   "--eval", "run_python",
                   "--eval-model", "sonnet",
                   "--eval-effort", "max",
                   "--quiet"],
...
def test_pipeline_override_eval_on_ladder(
    ...
    ladder = _ladder()
    with patch("splinter.pipeline.load_ladder", return_value=ladder), \
         patch("splinter.pipeline.get_strategy", side_effect=ValueError("stop early")):
        try:
            run_pipeline(task_path=str(task_file), eval_model="custom-eval",
                         eval_effort="low")
        except ValueError:
            pass
